...
export function getWorkingHoursDisplay(record) {
    if (record.workingHours != null) {
        if (record.workingHours > 24 || record.workingHours < 0) return "—";
    ...
    if (record.checkIn && !record.checkOut) {
        ...
        if (isNaN(checkInTime) || checkInTime <= 0) return "—";
        const diffMs = Math.max(0, Date.now() - checkInTime);
        const diffHours = Math.min(24, diffMs / (1000 * 60 * 60));
...
export function getDayTypeDisplay(record) {
    if (record.dayType && record.dayType !== "Absent") {
        let normalized = record.dayType;
        if (normalized === "Three Quarter Day") normalized = "Full Day";
        if (normalized === "Short Day") normalized = "Half Day";
        ...
        return {
            label: normalized,
            className: map[normalized] || "badge-warning",
        };
    }
    if (record.checkIn && !record.checkOut) {
